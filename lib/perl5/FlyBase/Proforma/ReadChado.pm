...
sub table2hash{
    ...
    if(!exists($primary_key{$table})){
        my $pristh=$dbh->primary_key_info('', '', $name);
        if(defined $pristh){
            while(my $priref=$pristh->fetchrow_hashref){
                  $primary_key{$table}=$priref->{COLUMN_NAME};
            }
        }
        else{
            print STDERR "ERROR: could not get primary key for table $table\n";
        }
    }
    if(!exists($columninfo{$table}){
        my $clsth=$dbh->column_info('','', $table,'');
        while(my $hashref=$clsth->fetchrow_hashref){
             $columninfo{$table}{$hashref->{COLUMN_NAME}}=$hashref->{IS_NULLABLE};
        }
    }
    my ( $ftables,$wherea,@binding)=compile_statement($dbh, $table, $columns);
    my $statement="select $table.* from $table $ftables where $wherea ";
    $statement=~s/and $//;
    my $hashref=$dbh->selectall_hashref($statement, $primary_key{$table},undef,@binding);
...
sub compile_statement{
    ...
    my $forsth=$dbh->foreign_key_info('','','','','',$table);
    if(defined $forsth){
        while(my $forref=$forsth->fetchrow_hashref){
            foreach my $key (keys %$forref){
               $foreign_key{$table."_".$forref->{FK_COLUMN_NAME}}=$forref->{UK_TABLE_NAME};
               $foreign_key{$table."_".$forref->{FK_COLUMN_NAME}}{column}=$forref{UK_COLUMN_NAME};
            } 
        }
    }
    foreach my $key (keys %$columns){
        if(ref($key) eq "HASH"){
            if(exists($foreign_key{$table."_".$key})){
                $ftables.=", ", $foreign_key{$table."_".$key}, " $table"."_".$key;
                $whereabout.=$table.".".$key."=".$table."_".$key.".".$foreign_key{$table."_".$key}{column}." and ";        my ($sta,$wab,@b)=compile_statement($dbh,$table."_".$key,$columns->{$key});
            }
        }
        else{
            $whereabout.=$table.".".$key=? ";
            push(@bind, $columns->{$key});
        }
    }
        
    return ($ftables, $whereabout, @bind);
}
sub get_feature{
    my %params=@_;
    my @result=();
