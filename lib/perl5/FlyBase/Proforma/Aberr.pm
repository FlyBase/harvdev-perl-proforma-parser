...
my %feat_type =
  ( 'A24a', 'transposable_element_insertion_site', 'A30', 'library', );
...
sub process {
    ...
    if ( exists( $fbcheck{ $ph{A1a} }{ $ph{pub} } ) ) {
        print STDERR
          "Warning: $ph{A1a} $ph{pub} exists in a previous proforma\n";
    }
    $fbcheck{ $ph{A1a} }{ $ph{pub} } =
      1;    # log the record, if this happens twice, it may be a duplicate.

    if ( !exists( $ph{A27b} ) ) {

        print STDERR
          "Action Items: aberration $unique == $ph{A1a} with pub $ph{pub}\n";
        ...
        if ( $f eq 'A1b' || $f eq 'A2b' ) {
            ...
            if ( $num != 0 ) {
                if (
                    ...
                    || ( !defined( $ph{A2c} ) && !defined( $ph{A1f} ) )
                  )
                {
                    print STDERR
                      "ERROR: A2a must have A2c filled in unless a merge\n";
                }
                else {
...
#$out.=write_feature_synonyms($doc,$unique,$ph{$f},'a','unattributed',$ti_fpr_type{$f});
            ...
            $out .=
              merge_records( $self->{db}, $unique, $ph{$f}, $ph{A1a}, $ph{pub},
                $ph{A2a} );
                ...
                foreach my $item (@items) {
                    ...
                    if ( $f eq 'A19a' ) {
                        if ( $item =~ /Inferred to overlap with: \@(.*)@/ ) {
                            ...
                            print STDERR
                              "Warning: format is not correct for $f\n";
                        }
                    }
                ...
                if ( $libu eq '0' ) {
                    ...
                    if ( defined( $ph{A30a} ) && $ph{A30a} ne "" ) {
                        if ( exists( $a30a_type{ $ph{A30a} } ) ) {
                            ...
                            print STDERR
                              "ERROR: wrong term for A30a $ph{A30a}\n";
                        }
                    }
                    else {
                        print STDERR
                          "ERROR: A30 has a library no term for A30a\n";
                    }
            ...
            if ( exists( $ph{'A17.upd'} ) && $ph{'A17.upd'} eq 'c' ) {
                ...
                foreach my $t (@result) {
                        ...
                        && $t->{type} eq 'aberr_pheno' )
                    {
                        ...
                        print STDERR
"ERROR: !c  phendesc  for $ph{A1a} $f  $ph{pub}, $ti_fpr_type{$f} NOT found\n";
                    }
                }
            ...
            if ( exists( $ph{"$f.upd"} ) ) {
                print STDERR
                  "ERROR: Wrong proforma using obsoleted field $f yet\n";
            }
            print STDERR
              "ERROR: Wrong proforma using obsoleted field $f  yet\n";
        }
                ...
                if ( $ph{A1a} =~ /^(D\w{3})\\/ ) {
                    ...
                    foreach my $t (@results) {
                        ...
                        if (
                            ...
                            || (
                                ...
                                && $num - $frnum{$unique}{ $ti_fpr_type{$f} }
                                { $t->{rank} } == 1
                            )
                          )
                        {
                    ...
                    foreach my $t (@frresults) {
                          ...
                          )
                        {
                            ...
                            $out .= delete_featureprop_pub( $doc, $t, $unique,
                                'inferred_cyto', $ph{pub} );
                        }
                        else {
                            print STDERR
                    ...
                    my @results =
                      get_unique_key_for_fr( $self->{db}, 'object_id',
                        'subject_id', $unique, $ti_fpr_type{$f}, $ph{pub} );
                    my $Nn = @results;
                        ...
                        if ( $num == 1 ) {
                            ...
                            $out .=
                              delete_feature_relationship_pub( $self->{db},
                                $doc,
                                $ta, 'object_id', 'subject_id', $unique,
                                $ti_fpr_type{$f}, $ph{pub} );
                            foreach my $stype ( 'progenitor', 'cyto_left_end',
                                'cyto_right_end' )
                            {
                                my @subs = get_unique_key_for_fr(
                                    $self->{db}, 'subject_id',
                                    'object_id', $ta->{name},
                                    $stype,      $ph{pub}
                                );
                                foreach my $st (@subs) {
                                    my $s_n = get_fr_pub_nums( $self->{db},
                                        $st->{fr_id} );
                                    if ( $s_n == 1 ) {
                                        $out .= delete_feature_relationship(
                                            $self->{db}, $doc,
                                            $st,         'subject_id',
                                            'object_id', $ta->{name},
                                            $stype
                                        ...
                                        $out .=
                                          delete_feature_relationship_pub(
                                            $self->{db}, $doc,
                                            $st,         'subject_id',
                                            'object_id', $ta->{name},
                                            $stype,      $ph{pub}
            ...
            if ( length( $ph{$f} ) > 1 ) {
                ...
                if ( $ph{A1a} =~ /^(D\w{3})\\/ ) {
                    ...
                    if ( $g ne '0' ) {
                        ...
                        $ph{$f} =~ s/\n/ /g;
                        $out .= write_featureprop( $self->{db}, $doc, $unique,
                            $ph{$f}, $br_type, $ph{pub} );
                ...
                if ( $ph{$f} =~ /\[\]/ ) {
                    print STDERR
                      "PROBLEM: check value of inferred_cyto, should be [][]\n";
                    ...
                    foreach my $item (@items) {
                        ...
                        if ( $item ne '' ) {
                            ...
                            if ( $left ne '' ) {
                                ...
                                $br_feature->dispose();
                                print STDERR
                                  "Create feat rel $unique $br_unique\n";
                                ...
                                if ( $left =~ /^P/ ) {
                                    ...
                                    if ( exists( $fbids{$left} ) ) {
                                        ...
                                        if ( $s_unique eq '0' ) {
                                            print STDERR
"ERROR: could not find uniquename for $left $f\n";
                                        }
                                        else {
                                    ...
                                    print STDERR
                                      "NOt starting P=> ==$left, $right==\n";
                                    if ( defined($left) && $left ne '' ) {
                                        $loc .=
                                          '[' . $left . '-' . $right . '];';
                                        my $left_unique = 'band-' . $left;
                                        my ( $o_g, $o_s, undef ) =
                                          get_feat_ukeys_by_uname( $self->{db},
                                            $left_unique );
                                        if ( $o_g eq '0' ) {
                                            print STDERR
"ERROR: --$left_unique-- could not found in DB\n";
                                        }
                                        my $leftfeature = create_ch_feature(
                                            doc        => $doc,
                                        ...
                                        if ( $s_g eq '0' ) {
                                            print STDERR
"ERROR: $right_unique could not found in DB\n";
                                        }
                                ...
                                print STDERR
"ERROR: Incorrect format assignment in '$item'. Expected (\\d+):(\\w+)-(\\w+)";
                            }
                        }
                    ...
                    if ( $loc ne '' ) {
                        $out .=
                          write_featureprop( $self->{db}, $doc, $unique, $loc,
                            'inferred_cyto', $ph{pub} );
                    }
...
sub write_feature {
    ...
    if ( exists( $ph{A1f} ) ) {
        if ( $ph{A1g} eq 'n' ) {
            print STDERR
              "Aberr Merge  A1g = n check: does A1a $ph{A1a} exist\n";
            my $va = validate_new_name( $db, $ph{A1a} );
            if ( $va == 1 ) {
                print STDERR
                  "ERROR: Aberr Merge  A1g = n and A1a $ph{A1a} exists\n";
                exit(0);
            }
        }
        my $tmp = $ph{A1f};
        ...
        if ( $ph{A1g} eq 'y' ) {
            ( $unique, $genus, $species, $type ) =
              get_feat_ukeys_by_name_type( $self->{db}, $ph{A1a},
            ...
            if ( $unique eq '0' or $unique eq '2' ) {
                ...
                if ( exists( $fbids{ $ph{A1a} } ) ) {
                    ...
                    if ( $unique ne $check ) {
                        print STDERR
"ERROR: $check and $unique are not same for $ph{G1a}\n";
            ...
            if ( exists( $ph{A1e} ) ) {
                if ( exists( $fbids{ $ph{A1e} } ) ) {
                    print STDERR
"ERROR: Rename A1e $ph{A1e} exists in a previous proforma\n";
                }
                if ( exists( $fbids{ $ph{A1a} } ) ) {
                    print STDERR
"ERROR: Rename A1a $ph{A1a} exists in a previous proforma \n";
                }
                print STDERR
                  "Action Items: rename aberration $ph{A1e} to $ph{A1a}\n";
                ( $unique, $genus, $species, $type ) =
                  get_feat_ukeys_by_name_type( $self->{db}, $ph{A1e},
                    'chromosome_structure_variation' );
                if ( $unique eq '0' or $unique eq '2' ) {
                    print STDERR
                      "ERROR: could not find $ph{A1e} in the database\n";
                }
                else {
                    $feature = create_ch_feature(
                        uniquename => $unique,
                ...
                if ( $va == 1 ) {
                    $flag = 0;
                    ( $unique, $genus, $species, $type ) =
                      get_feat_ukeys_by_name_type( $db, $ph{A1a},
                        'chromosome_structure_variation' );
                    $fbids{ $ph{A1a} } = $unique;
                }
                else {
                    ...
                    ( $unique, $flag ) = get_tempid( 'ab', $ph{A1a} );
                    print STDERR
                      "Action Items: new aberration $ph{A1a} $unique\n";
                    if ( $ph{A1a} =~ /^(.*)\\(.*)$/ ) {
                ...
                if ( $flag == 0 ) {
                    ...
                    print STDERR
                      "ERROR, name $ph{A1a} has been used in this load\n";
                }
            }
        }
    }
...
sub parse_segment {
    ...
    if ( exists( $gen{ $f . 'b.upd' } ) && $gen{ $f . 'b.upd' } eq 'c' ) {
        ...
        if ( @results == 0 ) {
            print STDERR
"ERROR: could not find previous records in the database $fbid, $pub_id $f b\n";
        }
        foreach my $t (@results) {
        ...
        && $gen{ $f . 'c' } ne '' )
    {
        ...
        $out .= write_featureprop( $db, $doc, $fbid, $value,
            $ti_fpr_type{ $f . 'a' }, $pub_id );
    }
    return $out;
}
...
sub parse_multiple_break {
    ...
    if ( defined( $gen_loc{A90a} ) && $gen_loc{A90a} ne '' ) {
        ...
        if ( $g ne '0' && $g ne '2' ) {
            print STDERR
"CHECK: chromosome_breakpoint $br_unique already in DB $g, $s, $ta\n";
        ...
        if ( exists( $gen_loc{'A90b.upd'} ) && $gen_loc{'A90b.upd'} eq 'c' ) {
            ...
            if ( @results == 0 ) {
                print STDERR
"ERROR: could not find previous records in the database $fbti,$br_unique $pub_id A90b\n";
            }
            foreach my $t (@results) {
...
#		      print STDERR "CHECK: chromosome_breakpoint $br_unique !c A90b >1 featureprop\n";
            ...
            print STDERR
"CHECK: chromosome_breakpoint $br_unique !c A90b delete featureloc\n";
        ...
        if ( defined( $gen_loc{A90b} ) && $gen_loc{A90b} ne '' ) {
            ...
            if ( defined($location) ) {
                ...
                print STDERR
"ERROR: Something wrong with A90b  $gen_loc{A90b} please fix \n";
            }
            if ( $fmin > $fmax ) {
            ...
            if ( defined( $gen_loc{A90c} ) ) {
                if ( $gen_loc{A90c} eq '4' ) {
                    $srcfeat = $arm . '_r4';
                    print STDERR
"ERROR WARN: A90c Release 4 will not display in Sequence coordinates\n";
                }
                elsif ( $gen_loc{A90c} eq '3' ) {
                    $srcfeat = $arm . '_r3';
                    print STDERR
"ERROR WARN: A90c Release 3 will not display in Sequence coordinates\n";

                }
                elsif ( $gen_loc{A90c} eq '5' ) {
                    $srcfeat = $arm . '_r5';
                    print STDERR
"ERROR WARN: A90c Release 5 will not display in Sequence coordinates\n";
            ...
            if ( exists( $gen_loc{A90c} ) && $gen_loc{A90c} eq '6' ) {
                ...
                if ( defined($fmin) && defined($fmax) ) {
                    ...
                    if ( !defined($group) ) {
                        $group =
                          &get_max_locgroup( $db, $br_unique, $arm, $fmin,
                            $fmax );
                    }
                    ...
                    print STDERR
"DEBUG A90c Release 6 create featureloc feature $br_unique srcfeature $arm fmin $fmin fmax $fmax locgroup $group\n";
                    ...
                    print STDERR
"DEBUG A90c Release 6 create featureloc feature $br_unique srcfeature $arm locgroup $group\n";
                }
                my $fl_pub =
                ...
                my $rank =
                  get_max_featureprop_rank( $db, $br_unique,
                    'reported_genomic_loc', $value,
                    'GenBank feature qualifier' );
                ...
                print STDERR "ERROR: No (A90c) Release number. This is now required."
            }
        }
