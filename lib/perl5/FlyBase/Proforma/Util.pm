...
sub check_al_with_fr_or_mutagen {
	...
	feature_relationship fr where f1.feature_id=fr.subject_id and
    ...
    if ( $num > 0 ) {
        ...
        my $state = "select cvterm.name from feature_cvterm fcv,
		cvterm,cvtermprop,
		feature where feature.uniquename='$fb_id' and
		feature.feature_id=fcv.feature_id and
		cvterm.cvterm_id=fcv.cvterm_id and
		cvterm.cvterm_id=cvtermprop.cvterm_id and cvtermprop.value =
		'mutagen' and cvterm.name like 'in vitro construct %';";
